...
%             fprintf('error e %f \n',alt_e-obj.e)
        ...
        function obj = Jacobian_function(obj)
            ...
            d_phi = MX.sym('v');    
            e = MX.sym('e');
            a_wheel_angle = MX.sym('a_wheel_angle');
            v_wheel_angle = MX.sym('v_wheel_angle'); 
            ...
            % Slip angles
            alpha_fl = MX.sym('alpha_fl');
            alpha_fr = MX.sym('alpha_fr');
            alpha_rl = MX.sym('alpha_rl');
            alpha_rr = MX.sym('alpha_rr');
            
            alpha_fl = -a_wheel_angle + atan2((v + a*r),u-track_w*r); %
            alpha_fr = -a_wheel_angle + atan2((v + a*r),u+track_w*r); %

            alpha_rl = atan2(v-b*r,u-track_w*r); %
            alpha_rr = atan2(v-b*r,u+track_w*r); %
...
%             force_rr(v, r, d_phi, e, a_wheel_angle) = -c_r.*alpha_rr;
            ...
            %% dot state vector dot_v dot_r dot_d_phi dot_e v_wheel_angle
            % dot_v
            f1 = ((1/mass)* ( (force_fl+force_fr)*cos(a_wheel_angle) + (force_rl+force_rr))) - r*u;
            % dot_r
            f2 = (a*(force_fl+force_fr)*cos(a_wheel_angle) - b*(force_rl+force_rr))/Izz;
            % dot_d_phi
            f3 =  r - k*u;
            % dot_e
            f4 = v + u * d_phi;%v + u * d_phi;
...
%             H_cost = hessian(cost,[v, r, d_phi, e, a_wheel_angle, v_wheel_angle]);
            ...
            J_sym = jacobian([f1,f2,f3,f4,f5],[v, r, d_phi, e, a_wheel_angle]);
            obj.J = Function('Jacobian',{[v, r, d_phi, e, a_wheel_angle, dot_v]},{J_sym});
        end
...
%             matlabFunction(car.J,'File','CalculateJ');
            ...
            a_wheel_angle = x(5);
            dot_v = dot_x(1);
            Jacobian_output = full(obj.J([v, r, d_phi, e, a_wheel_angle, dot_v]));
