...
%     Solver = 'hpipm';
    ...
    for i=2:NHorizon
        ...
        HorizonIter(i).Qk = diag([0,0,0.1,50,0]);
        HorizonIter(i).Rk = 50;
        HorizonIter(i).fk = zeros(5,1);%[0;0;-1;-1;0];
                           % v   r d_phi  e   a_wheel_angle v_wheel_angle
        HorizonIter(i).lb = [-5 -3 -1.5  -5.0  -0.5          -0.5]';
        HorizonIter(i).ub = [ 5  3  1.5   5.0   0.5           0.5]';
    end
    HorizonIter(NHorizon-1).Qk = diag([0,0,0.1,100,0]);
    HorizonIter(NHorizon).Qk = diag([0,0,0.1,100,0]);   
    ...
    elseif strcmp(Solver,'hpipm')
        [ X,U,info ] = HPIPMoptimizer_v3(HorizonIter,NHorizon);
    end
