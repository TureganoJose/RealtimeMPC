...
% https://www.researchgate.net/publication/308737646_From_linear_to_nonlinear_MPC_bridging_the_gap_via_the_real-time_iteration
%% Load track
load('.\Tracks\dlc2.mat') %dlc2 %track
TrackScale = 1;
...
% for i=1:150
%     logging.track_ref(i,:) = car.InterrogateNURBS(i);
% end
% plot(logging.track_ref(:,1), logging.track_ref(:,2), 'g.')
tSim = 5;
tHorizon = 1;
...
% 
% 
% % x_alt(:,:)= [pre_opt_state(1,:), pre_opt_state(1,:),
% 
% for i=1:NHorizon-1
...
logging.ay = zeros(NSim,1);

logging.track_ref = zeros(NSim,2);
...
%     
    ...
    logging.ay(iSim) = car_sim.dot_v + car_sim.r * car_sim.u;
    logging.track_ref(iSim,:) = car.InterrogateNURBS(logging.s(iSim));
    ...
    error_lateral(iSim)= norm(StateVariables(4,:) - X(4,:));
end
toc
